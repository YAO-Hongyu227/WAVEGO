...
double WALK_LIFT        = 20; // WALK_HEIGHT + WALK_LIFT <= WALK_HEIGHT_MAX.
...
void newsinglegait(uint8_t LegNum, float cycleInput, float directionInput, double target_IMU[3], double target_COM[3][1], bool crabwalk){
  ...
  if(cycleInput < WALK_LIFT_PROP)
  {
    ...
    //  :            .
    // :              . 
    if(WLP<0.5){ swing_height = (cos( (WLP * M_PI) + M_PI*3/2)) * WL; }
    else{ swing_height = (cos( (WLP - 0.5)* M_PI)) * WL; }
  ...
  //using right leg bias to control the yaw while walking
  if(LegNum == 3 || LegNum == 4) //right legs
  {
    x_distance = x_distance * right_leg_bias;
  }
  if(LegNum == 1 || LegNum == 2)  //left legs
  {
    x_distance = x_distance * ( 2 - right_leg_bias);
  }
...
//
                                                                          // this is poscom[3][1]
void trot_turninggait(uint8_t LegNum, float cycleInput, float directionInput, double target_IMU[3], double target_COM[3][1], float turningdirection){
...
float offset_x = 10; //the offset when circling the COM
float offset_z = 7;
float offset_cycleinput = 0; // the offset of layback time
float cTX=-1,cTY=1;
bool isclimb = false;


// free triangular walk gait
void newTriangularGait(float GlobalInput, float directionAngle, int turnCmd){
  float StepA;
  float StepB;
  float StepC;
  float StepD;
  ...
  //the term is A = B -> C = D -> A
  StepA = GlobalInput - 0.05 + 1;
  StepB = GlobalInput - 0.25 + 1 - 0.05;
  StepC = GlobalInput - 0.5 + 1 -0.05;
  StepD = GlobalInput - 0.75 + 1 - 0.05;
  ...
  Clock_X = GlobalInput + -0.05 + 1;
  if(Clock_X>1){Clock_X--;}
  //Clock_Y = Clock_X + 0.75;
  Clock_Y = Clock_X;
  if(Clock_Y>1){Clock_Y--;}

  if(Clock_X>0.15&&Clock_X<=0.25){cTX = -1;}
  if(Clock_Y>0.15&&Clock_Y<=0.25){cTY = besselCtrl(1,-1,(Clock_Y-0.15)*10);}

  if(Clock_X>0.4&&Clock_X<=0.5){cTX = besselCtrl(-1,1,(Clock_X-0.4)*10);}
  if(Clock_Y>0.4&&Clock_Y<=0.5){cTY = -1;}

  if(Clock_X>0.65&&Clock_X<=0.75){cTX = 1;}
  if(Clock_Y>0.65&&Clock_Y<=0.75){cTY = besselCtrl(-1,1,(Clock_Y-0.65)*10);}

  if(Clock_X>0.9&&Clock_X<=1){cTX = besselCtrl(1,-1,(Clock_X-0.9)*10);}
  if(Clock_Y>0.9&&Clock_Y<=1){cTY = 1;}

  // if(Clock_X>0&&Clock_X<=0.25){cTX = besselCtrl(-1,1,Clock_X*4);}
  // if(Clock_X>0.25&&Clock_X<=0.5){cTX = 1;}
  // if(Clock_X>0.5&&Clock_X<=0.75){cTX = besselCtrl(1,-1,(Clock_X-0.5)*4);}
  // if(Clock_X>0.75&&Clock_X<=1){cTX = -1;}

  // if(Clock_Y>0&&Clock_Y<=0.25){cTY = besselCtrl(-1,1,Clock_Y*4);}
  // if(Clock_Y>0.25&&Clock_Y<=0.5){cTY = 1;}
  // if(Clock_Y>0.5&&Clock_Y<=0.75){cTY = besselCtrl(1,-1,(Clock_Y-0.5)*4);}
  // if(Clock_Y>0.75&&Clock_Y<=1){cTY = -1;}
  ...
  if(DogNum == 3)
  {
    if(directionAngle>=20){right_leg_bias=1.13+0.21-0.1;}
      ...
      if (imu_on == true)
      {
        right_leg_bias = 0.93+0.21-0.23;
      }
      else{
        right_leg_bias = 0.93+0.21;
      } 
      }
  }

  right_leg_bias += RLB_bias;
...
void new_Trot_Turning_Gait(float GlobalInput, float directionAngle, float truningdirection){
  float StepA;
  float StepB;
  float StepC;
  float StepD;
  ...
  // if turnCmd = 1, do crab walk
  trot_turninggait(4, StepA, -directionAngle, PosIMU, PosCom, turning_direction);
  trot_turninggait(1, StepB, directionAngle, PosIMU, PosCom, turning_direction);
  trot_turninggait(2, StepC, directionAngle, PosIMU, PosCom, turning_direction);
  trot_turninggait(3, StepD, -directionAngle, PosIMU, PosCom, turning_direction);
...
void gaitTypeCtrl(float GlobalStepInput, float directionCmd, int turnCmd){
  ...
  else if(GAIT_TYPE == 11){
    newTriangularGait(GlobalStepInput, directionCmd, turnCmd);
  }  
}
...
//YAO, 7/24/2024, using newTriagularGait
// using newsinglegait
void functionAction_Triangular(){
    ...
    if(funcMode && GLOBAL_STEP == 0){
      ...
      switch(funcMode) {
        ...
        case 18:
          ...
          functionActionL();
        case 19:
          Serial.println("Action: Triangular Gait");
          functionAction_Triangular();
